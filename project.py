...
def run(folder, filterFile):
	'''runs the program'''
	
	names = os.listdir(folder)
	
	filter = getFilter(filterFile)
	animals = []
	for name in names:
		animals = animals + [Species(name, folder, filter)]
	
	print "comparing animals"
	
	diffMatrix = makeDiffMatrix(animals)
	tree = generateTree(diffMatrix, names)
...
def makeDiffMatrix(animals):
	...
	for n in range(0, totalSpecies-1):
		for m in range(n+1, totalSpecies):
			diffMatrix[n][m] = speciesCompare(animals[n], animals[m])
	return diffMatrix
